...
| AX-002 | arize-ax | Configure cloud auth without trial-and-error | langsmith | LangSmith accepts a single API key (+ optional URL) without an extra space header requirement. | Arize cloud setup uses `api_key` + `space_id`; env aliasing (`ARIZE_SPACE_KEY`/`ARIZE_SPACE_ID`) adds ambiguity. | Normalize to one cloud auth contract (Bearer or SDK-owned abstraction), and ship preflight validation that pinpoints missing fields. | new | P0 | `evals/platforms/arize.ts:132-145`, `evals/platforms/arize.ts:339-343`, `.env.example:114-117`, `evals/platforms/langsmith.ts:64-67`, `evals/gap-runs/2026-06-01T00-44-12-411Z.json` | 2026-06-01 | 2026-06-01 |
| AX-003 | arize-ax | Create datasets uniformly in local and cloud | langsmith | LangSmith exposes `createDataset(...)` directly in SDK setup flow. | Arize local path attempts best-effort dataset creation; cloud path requires dashboard/tabular API and skips symmetry. | Add a JS `createDatasetIfMissing` API that works for both Phoenix local and Arize cloud. | new | P0 | `evals/platforms/arize.ts:95-115`, `evals/platforms/langsmith.ts:101-106`, `evals/SETUP-FRICTION.md:7`, `evals/gap-runs/2026-06-01T00-44-12-411Z.json` | 2026-06-01 | 2026-06-01 |
| AX-004 | arize-ax | Confirm backend health before eval runs | braintrust | Braintrust ping path uses experiment summarize round-trip to verify real server responsiveness. | Arize ping treats `/healthz` 404 as potentially healthy, then relies on later span visibility to prove ingestion. | Add ingestion-backed health probe (emit probe span + confirm receipt) as the default readiness check. | new | P1 | `evals/platforms/arize.ts:163-177`, `evals/platforms/braintrust.ts:106-109`, `evals/SETUP-FRICTION.md:13`, `evals/gap-runs/2026-06-01T00-44-12-411Z.json` | 2026-06-01 | 2026-06-01 |
