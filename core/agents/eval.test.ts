...
describe('evaluateSandboxResults', () => {
  it('returns overallPass=true when tests pass (exit code 0, completed, not timed out)', () => {
    const result = evaluateSandboxResults(makeArtifact(), makeIssues(), 'Fixed it');
    expect(result.overallPass).toBe(true);
    expect(result.regressionDetected).toBe(false);
    expect(result.retryContext).toBeNull();
  });

  it('returns overallPass=false when exit code is non-zero', () => {
    const artifact = makeArtifact({ result: { exitCode: 1, stderr: 'FAIL: test_auth' } });
    const result = evaluateSandboxResults(artifact, makeIssues(), 'Fixed it');
    expect(result.overallPass).toBe(false);
    expect(result.retryContext).not.toBeNull();
  });

  it('returns overallPass=false when sandbox timed out', () => {
    const artifact = makeArtifact({ result: { timedOut: true, completed: false, exitCode: null } });
    const result = evaluateSandboxResults(artifact, makeIssues(), 'Fixed it');
    expect(result.overallPass).toBe(false);
  });

  it('returns overallPass=false when sandbox did not complete', () => {
    const artifact = makeArtifact({ result: { completed: false, exitCode: null } });
    const result = evaluateSandboxResults(artifact, makeIssues(), 'Fixed it');
    expect(result.overallPass).toBe(false);
  });

  it('always sets regressionDetected=false in Phase 1', () => {
    const artifact = makeArtifact({ result: { exitCode: 1 } });
    const result = evaluateSandboxResults(artifact, makeIssues(), 'Fixed it');
    expect(result.regressionDetected).toBe(false);
  });

  it('produces per-issue verdicts for all confirmed issues', () => {
    const issues = makeIssues(3);
    const result = evaluateSandboxResults(makeArtifact(), issues, 'Fixed it');
  ...
  it('all per-issue verdicts pass when overall passes', () => {
    const result = evaluateSandboxResults(makeArtifact(), makeIssues(), 'Fixed it');
    result.perIssueVerdicts.forEach((v) => expect(v.passed).toBe(true));
  });

  it('all per-issue verdicts fail when overall fails', () => {
    const artifact = makeArtifact({ result: { exitCode: 1, stderr: 'err' } });
    const result = evaluateSandboxResults(artifact, makeIssues(), 'Fixed it');
    result.perIssueVerdicts.forEach((v) => expect(v.passed).toBe(false));
  });

  it('includes stderr snippet in failure reason', () => {
    const artifact = makeArtifact({ result: { exitCode: 1, stderr: 'TypeError: cannot read property x of null' } });
    const result = evaluateSandboxResults(artifact, makeIssues(), 'Fixed it');
    expect(result.perIssueVerdicts[0].reason).toContain('TypeError');
  });

  it('retry context includes test command and exit code', () => {
    const artifact = makeArtifact({ result: { exitCode: 2, stderr: 'segfault' } });
    const result = evaluateSandboxResults(artifact, makeIssues(), 'Fixed it');
    expect(result.retryContext).toContain('npm test');
    expect(result.retryContext).toContain('Exit code: 2');
  });

  it('retry context includes stderr and stdout snippets', () => {
  ...
  it('prSummary mentions all issues on pass', () => {
    const result = evaluateSandboxResults(makeArtifact(), makeIssues(), 'Fix null check');
    expect(result.prSummary).toContain('#42');
    expect(result.prSummary).toContain('#43');
    expect(result.prSummary).toContain('Fix null check');
  });

  it('prSummary mentions failure on fail', () => {
    const artifact = makeArtifact({ result: { exitCode: 1, timedOut: true, completed: false } });
    const result = evaluateSandboxResults(artifact, makeIssues(), 'Fix null check');
...
describe('buildPRDetails', () => {
  it('title format: [agent-fix] {summary}', () => {
    const input = makeInput();
    const evalResult = evaluateSandboxResults(input.sandboxArtifact, input.confirmedIssues, input.fixSummary);
    const pr = buildPRDetails(input, evalResult);
    expect(pr.title).toBe('[agent-fix] Fix null check in parser');
  });

  it('body includes design summary', () => {
    const input = makeInput();
    const evalResult = evaluateSandboxResults(input.sandboxArtifact, input.confirmedIssues, input.fixSummary);
    const pr = buildPRDetails(input, evalResult);
  ...
  it('body includes per-issue verdicts', () => {
    const input = makeInput();
    const evalResult = evaluateSandboxResults(input.sandboxArtifact, input.confirmedIssues, input.fixSummary);
    const pr = buildPRDetails(input, evalResult);
  ...
  it('body includes sandbox run link', () => {
    const input = makeInput();
    const evalResult = evaluateSandboxResults(input.sandboxArtifact, input.confirmedIssues, input.fixSummary);
    const pr = buildPRDetails(input, evalResult);
  ...
  it('body includes retry information when retryCount > 0', () => {
  ...
  it('body does not include retry section when retryCount is 0', () => {
  ...
  it('labels include agent-fix plus issue type labels', () => {
  ...
  it('labels deduplicate issue types', () => {
  ...
  it('head format: forkOrg:branchName', () => {
  ...
  it('base is the upstream default branch', () => {
  ...
  it('does not auto-assign reviewers', () => {
    const input = makeInput();
    const evalResult = evaluateSandboxResults(input.sandboxArtifact, input.confirmedIssues, input.fixSummary);
    const pr = buildPRDetails(input, evalResult);
...
describe('routeEvalResult', () => {
  it('routes to open_pr when overall passes', () => {
    const evalResult = evaluateSandboxResults(makeArtifact(), makeIssues(), 'Fixed');
    const routing = routeEvalResult(evalResult, 0, 3);
    expect(routing.action).toBe('open_pr');
  });

  it('routes to retry when overall fails and retries remain', () => {
    const artifact = makeArtifact({ result: { exitCode: 1, stderr: 'fail' } });
    const evalResult = evaluateSandboxResults(artifact, makeIssues(), 'Fixed');
    const routing = routeEvalResult(evalResult, 0, 3);
    expect(routing.action).toBe('retry');
  });

  it('routes to retry when retryCount < maxRetries', () => {
    const artifact = makeArtifact({ result: { exitCode: 1, stderr: 'fail' } });
    const evalResult = evaluateSandboxResults(artifact, makeIssues(), 'Fixed');
  ...
  it('routes to failed when retryCount >= maxRetries', () => {
    const artifact = makeArtifact({ result: { exitCode: 1, stderr: 'fail' } });
    const evalResult = evaluateSandboxResults(artifact, makeIssues(), 'Fixed');
  ...
  it('retry routing includes retry context', () => {
    const artifact = makeArtifact({ result: { exitCode: 1, stderr: 'error msg' } });
    const evalResult = evaluateSandboxResults(artifact, makeIssues(), 'Fixed');
    const routing = routeEvalResult(evalResult, 0, 3);
    expect(routing.action).toBe('retry');
    if (routing.action === 'retry') {
      expect(routing.retryContext).toContain('Exit code: 1');
    }
  });

  it('failed routing includes max retries info', () => {
    const artifact = makeArtifact({ result: { exitCode: 1, stderr: 'err' } });
    const evalResult = evaluateSandboxResults(artifact, makeIssues(), 'Fixed');
    const routing = routeEvalResult(evalResult, 3, 3);
  ...
  it('routes to failed with maxRetries=0 on first failure', () => {
    const artifact = makeArtifact({ result: { exitCode: 1, stderr: 'err' } });
    const evalResult = evaluateSandboxResults(artifact, makeIssues(), 'Fixed');
