...
export function evaluateRetryDecision(
  ...
  if (currentAttemptCount < config.maxRetries) {
    ...
    // combinedRetryContext should include *previous* attempts only.
    // In the main pipeline (runRetryLoop) the latest failure is recorded before evaluation,
    // so we exclude the attempt whose retryContext matches latestRetryContext when present.
    const matchIndex = history.attempts.findIndex((a) => a.retryContext === latestRetryContext);
    const combinedAttempts = matchIndex >= 0
      ? history.attempts.slice(0, matchIndex)
      : history.attempts;
    const combinedContext = buildCombinedRetryContext(combinedAttempts);
